...
int tcp_client(unsigned short port)
{
	...
	if (port > 0)
	{
		...
		localaddr.sin_addr.s_addr = inet_addr(tunable_listen_address);
...
const char * get_file_modify_time(const struct stat * sbuf)
{
	...
	if (sbuf->st_mtime > local_time || (local_time - sbuf->st_mtime  > 60 * 60 * 24 * 181))
	{
		p_date_format = "%b %e  %Y";
	}
...
static int lock_file(const int fd, const short lock_type)
{
	int ret;
	struct flock lock;
